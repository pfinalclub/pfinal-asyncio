...
function create_task(callable $callback, string $name = ''): Task
{
    ...
    if ($scope === null) {
        // 开发模式：提供更详细的错误信息
        $debugHint = (getenv('ASYNCIO_DEBUG') || (defined('ASYNCIO_DEBUG') && ASYNCIO_DEBUG)) 
            ? "\n提示: run() 函数会自动创建 Scope。如果需要在 run() 之外创建任务，请手动使用 CancellationScope::run()。"
            : "";
        
        throw new \RuntimeException(
            "No active CancellationScope. Use CancellationScope::run() to create a scope for tasks." . $debugHint
        );
    }
